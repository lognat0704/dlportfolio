...
def contextual_attention(f, b, mask=None, ksize=3, stride=1, rate=1,
    ...
    My understanding of contextual attention:
       1) cut original bg into 4*4 patches, with stride [1,2,2,1], and reshape them to make num_of_patch1 feature maps of 4*4*3
       2) shrink original bg and fg and mask (if given) by 50% (as rate = 2)
       3) cut shrinked bg into 3*3 patches, with stride [1,1,1,1], and reshape them to make num_of_patch2 feature maps of 3*3*3
       ? num_of_patch1 = num_of_batch2 (yes!!!, it's because the different stride and original bg h and w are even numbers)
       4) apply shrink bg patch feature maps to shrinked fg. In other words, for each 3*3 patch in shrinked fg, find its similarity with the num_of_patch2 3*3 patches of shrinked bg
       5) apply mask and softmax to the similiarity in previous step, so if the patch in shrinked bg is masked, its similarity with fg patch goes to 0
       6) fuse the similarity (see paper for details)
       7) reconstruct fg by upscaling (conv2d_transpose) using the similarity and original bg patch feature map
    ...
    m = m[0] ## only keep the first in batch dimension
    ...
    for xi, wi, raw_wi in zip(f_groups, w_groups, raw_w_groups):
        ## xi:h*w*c / wi: k*k*c*num_of_patches / raw_wi: k*k*c*num_of_patches
        # conv for compare
        wi = wi[0]
        wi_normed = wi / tf.maximum(tf.sqrt(tf.reduce_sum(tf.square(wi), axis=[0,1,2])), 1e-4)
        yi = tf.nn.conv2d(xi, wi_normed, strides=[1,1,1,1], padding="SAME") ## yi: new_h*new_w*num_of_patches 
        ...
        if fuse:
            ...
            yi = tf.transpose(yi, [0, 2, 1, 4, 3])
        yi = tf.reshape(yi, [1, fs[1], fs[2], bs[1]*bs[2]]) ## yi: 1*h*w*hw
        ...
        ## conv2d_transpose:
        ## value的格式为[batch, height, width, in_channels]
        ## filter的格式为[height, width, output_channels, input_channels]，务必注意这里的channel数是输出的channel数在前，输入的channel数在后
        ## output_shape是一个1-D张量，传入的可以是一个tuple或者list，在不指定data_format参数的情况下，格式必须为NHWC
        yi = tf.nn.conv2d_transpose(value = yi, filter = wi_center, output_shape = tf.concat([[1], raw_fs[1:]], axis=0), strides=[1,rate,rate,1]) / 4.
        y.append(yi)
...
def test_contextual_attention(args):
    ...
    cv2.imwrite(args.imageOut, np.concatenate([f[0],y[0][0].astype(np.uint8)], axis = 1))
...
def dis_sn_conv(x, cnum, ksize=5, stride=2, name='conv', training=True):
...
# https://github.com/avalonstrel/GatedConvolution/blob/master/inpaint_model_gc.py
def gan_sn_pgan_loss(pos, neg, summary, name='sn_pgan_loss'):
    """
    ...
    with tf.variable_scope(name):
        ...
        if summary: ## only track G training, not D training
...
def random_ff_mask(config, name="ff_mask"):
    ...
    Returns:
        tuple: (top, left, height, width)
    """
    ## it seems tf.py_func can't find function npmas defined outside random_ff_mask
    def npmask(IMG_SHAPES, MAXVERTEX, MAXANGLE, MAXLENGTH, MAXBRUSHWIDTH):
        h, w, _ = IMG_SHAPES
        
        mask = np.zeros((h,w), dtype = np.float32)
        num_v = 8+np.random.randint(MAXVERTEX)
        ...
        for i in range(num_v):
            ...
            for j in range(1+np.random.randint(5)):
                ...
                length = 10+np.random.randint(MAXLENGTH)
                brush_w = 10+np.random.randint(MAXBRUSHWIDTH)
                
                end_x = (start_x + length * np.sin(angle)).astype(np.int32)
                end_y = (start_y + length * np.cos(angle)).astype(np.int32)
                
                ## has to use 255 represent white color, not 1
                cv2.line(mask, (start_y, start_x), (end_y, end_x), 255, brush_w)
                cv2.circle(mask, (end_y, end_x), brush_w//2, 255, -1)
    ...
    with tf.variable_scope(name), tf.device('/cpu:0'):
        mask = tf.py_func(npmask, [config.IMG_SHAPES, config.MAXVERTEX, config.MAXANGLE, config.MAXLENGTH, config.MAXBRUSHWIDTH], tf.float32, stateful=False) / 255
        mask.set_shape([1] + config.IMG_SHAPES[:2] + [1])
    return mask


def npmask(config):
    """
    create a random numpy array with shape(height, width) and values {0, 255}
    
    """
    h, w, _ = config.IMG_SHAPES
    
    mask = np.zeros((h,w), dtype = np.float32)
    num_v = 8+np.random.randint(config.MAXVERTEX)
...
def create_batch_random_ff_masks(n):
    """ failed to find a way generate random mask for each mini-batch data
       originally random mask function is invoked in build_graph_with_losses().
       however this function is only invoked before training starts. after it, it won't be invoked anymore
       
       alternative approach, create those masks before training and read mask images same way as input images
       
       usage: 
       from inpaint_ops import create_batch_random_ff_masks
       
       create_batch_random_ff_masks(5)
    """
    import uuid
    config = ng.Config('inpaint.yml')
        
    for i in range(n):
        cv2.imwrite('masks/' + uuid.uuid4().hex + '.jpg', npmask(config))
